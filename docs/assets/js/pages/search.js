...
import { loadPhotos, routePhoto } from '../photos.js';
...
function resultCard(j, ctx) {
  ...
  /* No photograph on the result row. Measured: no rail results page in the
     surveyed set uses imagery, and a Google Flights results page returns zero
     images — a picture here competes with departure time, duration, changes and
     price, which is everything the row exists to convey. The photograph of the
     service appears once the row is expanded, beside the itinerary. */
...
function buildFares(mount, j, ctx) {
  ...
  /* Expanded: now the photograph earns its place, next to the itinerary the
     traveller is reading rather than the row they are scanning. */
  const pic = routePhoto(j.legs[0].trip.route.n, { alt: '', sizes: '(max-width:900px) 100vw, 300px' });
  mount.append(el('div', { class: 'itinerary-split' },
    el('div', {}, itineraryDetail(j)),
    pic ? el('div', { class: 'media itinerary-photo' }, pic) : null));
  ...
  mount.append(el('p', { class: 'note note-info mt-6', style: 'font-size:var(--step--2)' },
    el('strong', {}, 'Demonstration fares. '),
    'Timetables are real. Prices here are simulated.'));
}
